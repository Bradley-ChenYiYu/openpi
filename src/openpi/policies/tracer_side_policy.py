...
def make_tracer_side_example() -> dict:
    """Creates a random input example for the Tracer side-camera policy."""
    return {
...
class TracerSideInputs(transforms.DataTransformFn):
    """Convert tracer side-camera inputs into the model's expected input schema."""
    ...
    def __call__(self, data: dict) -> dict:
        ...
        inputs = {
            ...
            "image": dict(zip(names, (front_image, left_image, right_image), strict=True)),
...
@dataclasses.dataclass(frozen=True)
class TracerSideOutputs(transforms.DataTransformFn):
